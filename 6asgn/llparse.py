...
def first(s, productions, nonterminals):               # calculates first set
    ans = []
    if s in nonterminals:
        for st in productions[s]:
            if st[0] in terminals:                    # If terminal in production stop and move to next production
                ...
                j = 0
                sub_first = first(st[j], productions, nonterminals)
                for i in sub_first:
                    if i != epsil:
                        ans.append(i)
                j += 1
...
def left_recursion(productions):
    perfect = {}  # Dictionary having productions not starting with same non-terminal

    def calcPerfect(productions):  # Calculates perfect when grammar is updated
